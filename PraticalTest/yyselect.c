/*
generated at Sun May 26 22:55:01 2019
...
static short yynts_25[] = { yysexpr_NT, yysexpr_NT, 0 };
static short yynts_26[] = { yysassign_NT, yylv_NT, 0 };
static short yynts_27[] = { yybase_if_NT, 0 };
static short yynts_28[] = { yycond_NT, yystmt_NT, 0 };
static short yynts_29[] = { yybase_else_NT, 0 };
static short yynts_30[] = { yyif_NT, yystmt_NT, 0 };
static short yynts_31[] = { yybase_while_NT, 0 };
static short yynts_32[] = { yydo_NT, yyexpr_NT, 0 };
static short yynts_33[] = { yystart_NT, yystmt_NT, 0 };
static short yynts_34[] = { yybase_for_NT, 0 };
static short yynts_35[] = { yyexpr_NT, yyfor_NT, 0 };
static short yynts_36[] = { yyin_for_NT, yyend_for_NT, 0 };
static short yynts_37[] = { yycorpo_for_NT, yyexpr_NT, 0 };
static short yynts_38[] = { yystmt_NT, 0 };
static short yynts_39[] = { yystart_for_NT, yyexpr_NT, 0 };
...
/* 78 */	"rexpr: t_FACT(iexpr)",
/* 79 */	"sexpr: STR",
/* 80 */	"iexpr: EQ(sexpr,sexpr)",
/* 81 */	"iexpr: NE(sexpr,sexpr)",
/* 82 */	"iexpr: LT(sexpr,sexpr)",
/* 83 */	"iexpr: LE(sexpr,sexpr)",
/* 84 */	"iexpr: GT(sexpr,sexpr)",
/* 85 */	"iexpr: GE(sexpr,sexpr)",
/* 86 */	"sexpr: ATR(sassign,lv)",
/* 87 */	"sexpr: PTR(lv)",
/* 88 */	"sexpr: PTR(iexpr)",
/* 89 */	"sassign: sexpr",
/* 90 */	"sassign: iexpr",
/* 91 */	"base: base_if",
/* 92 */	"base_if: IF(cond,stmt)",
/* 93 */	"cond: expr",
/* 94 */	"base: base_else",
/* 95 */	"base_else: ELSE(if,stmt)",
/* 96 */	"if: IF(cond,stmt)",
/* 97 */	"base: base_while",
/* 98 */	"base_while: WHILE(do,expr)",
/* 99 */	"do: DO(start,stmt)",
/* 100 */	"start: START",
/* 101 */	"base: base_for",
/* 102 */	"base_for: t_END(expr,for)",
/* 103 */	"for: FOR(in_for,end_for)",
/* 104 */	"end_for: t_END(corpo_for,expr)",
/* 105 */	"corpo_for: stmt",
/* 106 */	"in_for: IN(start_for,expr)",
/* 107 */	"start_for: START",
/* 108 */	"stmt: BREAK",
/* 109 */	"stmt: CONTINUE",
/* 110 */	"cond: LT(iexpr,iexpr)",
/* 111 */	"cond: GT(iexpr,iexpr)",
/* 112 */	"cond: LE(iexpr,iexpr)",
/* 113 */	"cond: GE(iexpr,iexpr)",
/* 114 */	"cond: EQ(iexpr,iexpr)",
/* 115 */	"cond: NE(iexpr,iexpr)",
};
...
static void yyclosure_stmt(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 105, c + 1, p->cost[yycorpo_for_NT]);
...
static void yyclosure_expr(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 93, c + 1, p->cost[yycond_NT]);
...
static void yyclosure_iexpr(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 90, c + 1, p->cost[yysassign_NT]);
...
static void yyclosure_sexpr(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 89, c + 1, p->cost[yysassign_NT]);
	if (c + 1 < p->cost[yysassign_NT]) {
		p->cost[yysassign_NT] = c + 1;
		p->rule.yysassign = 1;
	}
...
static void yyclosure_base_if(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 91, c + 1, p->cost[yybase_NT]);
	if (c + 1 < p->cost[yybase_NT]) {
		p->cost[yybase_NT] = c + 1;
		p->rule.yybase = 3;
		yyclosure_base(a, c + 1);
...
static void yyclosure_base_while(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 97, c + 1, p->cost[yybase_NT]);
	if (c + 1 < p->cost[yybase_NT]) {
		p->cost[yybase_NT] = c + 1;
		p->rule.yybase = 5;
		yyclosure_base(a, c + 1);
...
static void yyclosure_base_for(NODEPTR_TYPE a, int c) {
	struct yystate *p = (struct yystate *)STATE_LABEL(a);
	yytrace(a, 101, c + 1, p->cost[yybase_NT]);
	if (c + 1 < p->cost[yybase_NT]) {
		p->cost[yybase_NT] = c + 1;
		p->rule.yybase = 6;
		yyclosure_base(a, c + 1);
...
static void yylabel(NODEPTR_TYPE a, NODEPTR_TYPE u) {
	...
	case 40: /* t_OPENP */
		...
		if (	/* finit: t_OPENP(blocop,NIL) */
			OP_LABEL(RIGHT_CHILD(a)) == 297 /* NIL */
		) {
	...
	case 59: /* t_END */
		...
		yytrace(a, 102, c + 0, p->cost[yybase_for_NT]);
		...
		yytrace(a, 104, c + 0, p->cost[yyend_for_NT]);
	...
	case 60: /* LT */
		...
		/* cond: LT(iexpr,iexpr) */
		c = ((struct yystate *)STATE_LABEL(LEFT_CHILD(a)))->cost[yyiexpr_NT] + ((struct yystate *)STATE_LABEL(RIGHT_CHILD(a)))->cost[yyiexpr_NT] + 1;
		yytrace(a, 110, c + 0, p->cost[yycond_NT]);
	...
	case 61: /* EQ */
		...
		/* cond: EQ(iexpr,iexpr) */
		c = ((struct yystate *)STATE_LABEL(LEFT_CHILD(a)))->cost[yyiexpr_NT] + ((struct yystate *)STATE_LABEL(RIGHT_CHILD(a)))->cost[yyiexpr_NT] + 1;
		yytrace(a, 114, c + 0, p->cost[yycond_NT]);
		if (c + 0 < p->cost[yycond_NT]) {
			p->cost[yycond_NT] = c + 0;
			p->rule.yycond = 6;
		}
	...
	case 62: /* GT */
		...
		/* cond: GT(iexpr,iexpr) */
		c = ((struct yystate *)STATE_LABEL(LEFT_CHILD(a)))->cost[yyiexpr_NT] + ((struct yystate *)STATE_LABEL(RIGHT_CHILD(a)))->cost[yyiexpr_NT] + 1;
		yytrace(a, 111, c + 0, p->cost[yycond_NT]);
		if (c + 0 < p->cost[yycond_NT]) {
			p->cost[yycond_NT] = c + 0;
			p->rule.yycond = 3;
		}
	...
	case 260: /* STR */
		/* sexpr: STR */
		yytrace(a, 79, 1 + 0, p->cost[yysexpr_NT]);
	...
	case 261: /* DO */
		...
		yytrace(a, 99, c + 0, p->cost[yydo_NT]);
	...
	case 262: /* WHILE */
		...
		yytrace(a, 98, c + 0, p->cost[yybase_while_NT]);
	...
	case 263: /* IF */
		...
		yytrace(a, 92, c + 0, p->cost[yybase_if_NT]);
		...
		/* if: IF(cond,stmt) */
		c = ((struct yystate *)STATE_LABEL(LEFT_CHILD(a)))->cost[yycond_NT] + ((struct yystate *)STATE_LABEL(RIGHT_CHILD(a)))->cost[yystmt_NT] + 1;
		yytrace(a, 96, c + 0, p->cost[yyif_NT]);
	...
	case 265: /* FOR */
		...
		yytrace(a, 103, c + 0, p->cost[yyfor_NT]);
	...
	case 266: /* IN */
		...
		yytrace(a, 106, c + 0, p->cost[yyin_for_NT]);
	...
	case 270: /* BREAK */
		/* stmt: BREAK */
		yytrace(a, 108, 1 + 0, p->cost[yystmt_NT]);
	...
	case 271: /* CONTINUE */
		/* stmt: CONTINUE */
		yytrace(a, 109, 1 + 0, p->cost[yystmt_NT]);
		if (1 + 0 < p->cost[yystmt_NT]) {
			p->cost[yystmt_NT] = 1 + 0;
			p->rule.yystmt = 3;
			yyclosure_stmt(a, 1 + 0);
	...
	case 281: /* ELSE */
		...
		yytrace(a, 95, c + 0, p->cost[yybase_else_NT]);
	...
	case 282: /* ATR */
		...
		c = ((struct yystate *)STATE_LABEL(LEFT_CHILD(a)))->cost[yysassign_NT] + ((struct yystate *)STATE_LABEL(RIGHT_CHILD(a)))->cost[yylv_NT] + (isString(a));
		yytrace(a, 86, c + 0, p->cost[yysexpr_NT]);
	...
	case 283: /* NE */
		...
		/* cond: NE(iexpr,iexpr) */
		c = ((struct yystate *)STATE_LABEL(LEFT_CHILD(a)))->cost[yyiexpr_NT] + ((struct yystate *)STATE_LABEL(RIGHT_CHILD(a)))->cost[yyiexpr_NT] + 1;
		yytrace(a, 115, c + 0, p->cost[yycond_NT]);
		if (c + 0 < p->cost[yycond_NT]) {
			p->cost[yycond_NT] = c + 0;
			p->rule.yycond = 7;
		}
	...
	case 284: /* GE */
		...
		/* cond: GE(iexpr,iexpr) */
		c = ((struct yystate *)STATE_LABEL(LEFT_CHILD(a)))->cost[yyiexpr_NT] + ((struct yystate *)STATE_LABEL(RIGHT_CHILD(a)))->cost[yyiexpr_NT] + 1;
		yytrace(a, 113, c + 0, p->cost[yycond_NT]);
		if (c + 0 < p->cost[yycond_NT]) {
			p->cost[yycond_NT] = c + 0;
			p->rule.yycond = 5;
		}
	...
	case 285: /* LE */
		...
		/* cond: LE(iexpr,iexpr) */
		c = ((struct yystate *)STATE_LABEL(LEFT_CHILD(a)))->cost[yyiexpr_NT] + ((struct yystate *)STATE_LABEL(RIGHT_CHILD(a)))->cost[yyiexpr_NT] + 1;
		yytrace(a, 112, c + 0, p->cost[yycond_NT]);
		if (c + 0 < p->cost[yycond_NT]) {
			p->cost[yycond_NT] = c + 0;
			p->rule.yycond = 4;
		}
	...
	case 293: /* PTR */
		...
		yytrace(a, 87, c + 0, p->cost[yysexpr_NT]);
		if (c + 0 < p->cost[yysexpr_NT]) {
			p->cost[yysexpr_NT] = c + 0;
			p->rule.yysexpr = 3;
			yyclosure_sexpr(a, c + 0);
		...
		yytrace(a, 88, c + 0, p->cost[yysexpr_NT]);
		if (c + 0 < p->cost[yysexpr_NT]) {
			p->cost[yysexpr_NT] = c + 0;
			p->rule.yysexpr = 4;
			yyclosure_sexpr(a, c + 0);
	...
	case 295: /* START */
		/* start: START */
		yytrace(a, 100, 1 + 0, p->cost[yystart_NT]);
		if (1 + 0 < p->cost[yystart_NT]) {
			p->cost[yystart_NT] = 1 + 0;
			p->rule.yystart = 1;
		}
		/* start_for: START */
		yytrace(a, 107, 1 + 0, p->cost[yystart_for_NT]);
...
static void yykids(NODEPTR_TYPE p, int eruleno, NODEPTR_TYPE kids[]) {
	...
	switch (eruleno) {
	case 115: /* cond: NE(iexpr,iexpr) */
	case 114: /* cond: EQ(iexpr,iexpr) */
	case 113: /* cond: GE(iexpr,iexpr) */
	case 112: /* cond: LE(iexpr,iexpr) */
	case 111: /* cond: GT(iexpr,iexpr) */
	case 110: /* cond: LT(iexpr,iexpr) */
	case 106: /* in_for: IN(start_for,expr) */
	case 104: /* end_for: t_END(corpo_for,expr) */
	case 103: /* for: FOR(in_for,end_for) */
	case 102: /* base_for: t_END(expr,for) */
	case 99: /* do: DO(start,stmt) */
	case 98: /* base_while: WHILE(do,expr) */
	case 96: /* if: IF(cond,stmt) */
	case 95: /* base_else: ELSE(if,stmt) */
	case 92: /* base_if: IF(cond,stmt) */
	case 86: /* sexpr: ATR(sassign,lv) */
	case 85: /* iexpr: GE(sexpr,sexpr) */
	case 84: /* iexpr: GT(sexpr,sexpr) */
	case 83: /* iexpr: LE(sexpr,sexpr) */
	case 82: /* iexpr: LT(sexpr,sexpr) */
	case 81: /* iexpr: NE(sexpr,sexpr) */
	case 80: /* iexpr: EQ(sexpr,sexpr) */
	...
	case 1: /* finit: t_OPENP(blocop,params) */
		...
		break;
	case 88: /* sexpr: PTR(iexpr) */
	case 87: /* sexpr: PTR(lv) */
	case 78: /* rexpr: t_FACT(iexpr) */
	...
	case 2: /* finit: t_OPENP(blocop,NIL) */
		kids[0] = LEFT_CHILD(p);
		break;
	case 105: /* corpo_for: stmt */
	case 101: /* base: base_for */
	case 97: /* base: base_while */
	case 94: /* base: base_else */
	case 93: /* cond: expr */
	case 91: /* base: base_if */
	case 90: /* sassign: iexpr */
	case 89: /* sassign: sexpr */
	...
	case 108: /* stmt: BREAK */
	case 107: /* start_for: START */
	case 100: /* start: START */
	case 79: /* sexpr: STR */
...
{lbl++; fprintf(outfp, pfRODATA pfALIGN pfLABEL, mklbl(lbl)); 
	...
	case 80: /* iexpr: EQ(sexpr,sexpr) */
#line 234 "diy.brg"
{ fprintf(outfp, pfCALL pfTRASH pfPUSH pfIMM pfEQ, "_strcmp", 8, 0); }
		break;
	case 81: /* iexpr: NE(sexpr,sexpr) */
#line 235 "diy.brg"
{ fprintf(outfp, pfCALL pfTRASH pfPUSH pfIMM pfNE, "_strcmp", 8, 0);}
		break;
	case 82: /* iexpr: LT(sexpr,sexpr) */
#line 236 "diy.brg"
{ fprintf(outfp, pfCALL pfTRASH pfPUSH pfIMM pfLT, "_strcmp", 8, 0); }
		break;
	case 83: /* iexpr: LE(sexpr,sexpr) */
#line 237 "diy.brg"
{ fprintf(outfp, pfCALL pfTRASH pfPUSH pfIMM pfLE, "_strcmp", 8, 0); }
		break;
	case 84: /* iexpr: GT(sexpr,sexpr) */
#line 238 "diy.brg"
{ fprintf(outfp, pfCALL pfTRASH pfPUSH pfIMM pfGT, "_strcmp", 8, 0); }
		break;
	case 85: /* iexpr: GE(sexpr,sexpr) */
#line 239 "diy.brg"
{ fprintf(outfp, pfCALL pfTRASH pfPUSH pfIMM pfGE, "_strcmp", 8, 0); }
		break;
	case 86: /* sexpr: ATR(sassign,lv) */
#line 242 "diy.brg"
{fprintf(outfp, pfSTORE);}
...
{ fprintf(outfp, pfJMP, mklbl(cycleControlers[cycleIndex-p->value.i].cont) ); }
	...
	case 110: /* cond: LT(iexpr,iexpr) */
#line 286 "diy.brg"
{  p->place = ++lbl; fprintf(outfp, pfJGE, mklbl(lbl)); }
		break;
	case 111: /* cond: GT(iexpr,iexpr) */
#line 287 "diy.brg"
{  p->place = ++lbl; fprintf(outfp, pfJLE, mklbl(lbl)); }
		break;
	case 112: /* cond: LE(iexpr,iexpr) */
#line 288 "diy.brg"
{  p->place = ++lbl; fprintf(outfp, pfJGT, mklbl(lbl)); }
		break;
	case 113: /* cond: GE(iexpr,iexpr) */
#line 289 "diy.brg"
{  p->place = ++lbl; fprintf(outfp, pfJLT, mklbl(lbl)); }
		break;
	case 114: /* cond: EQ(iexpr,iexpr) */
#line 290 "diy.brg"
{  p->place = ++lbl; fprintf(outfp, pfJNE, mklbl(lbl)); }
		break;
	case 115: /* cond: NE(iexpr,iexpr) */
#line 291 "diy.brg"
{  p->place = ++lbl; fprintf(outfp, pfJEQ, mklbl(lbl)); }
		break;
	default: break;
  }
}
...
//  fprintf(outfp, pfEXTRN, "_potencia");
